...
static SINavigationMenuView* menu = nil;
+ (SINavigationMenuView*)sharedMenuWithFrame:(CGRect)frame{
    if (!menu) {
        menu = [[SINavigationMenuView alloc] initWithFrame:frame title:@"自选"];
...
- (void)onShowMenu
{
    if (!self.table) {
        ...
        self.table = [SIMenuTable sharedTableWithFrame: CGRectMake(0, 64, self.menuContainer.frame.size.width, self.menuContainer.frame.size.height-64-49) items:self.items];
        KTLog(@"%@",NSStringFromCGRect(self.table.frame));
