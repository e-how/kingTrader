...
-(void)drawSerie:(Chart *)chart serie:(NSMutableDictionary *)serie{
   ...
   // NSLog(@" draw Serie name: %@ forData : %@", [serie objectForKey:@"name"], [serie objectForKey:@"data"]);
    ...
    if(chart.selectedIndex!=-1 && chart.selectedIndex < data.count && [data objectAtIndex:chart.selectedIndex]!=nil){
        ...
        CGContextSetStrokeColorWithColor(context, [[UIColor alloc] initWithRed:0.31 green:.31 blue:0.31 alpha:1.0].CGColor);
    ...
    CGContextBeginPath(context);
    CGContextSetRGBFillColor(context, R, G, B, 0.5);
