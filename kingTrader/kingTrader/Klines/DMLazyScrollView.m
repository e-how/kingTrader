...
- (KTStockDetailViewController *) visibleViewController {
...
- (KTStockDetailViewController *) viewControllerFromView:(UIView*) targetView {
    return (KTStockDetailViewController *)[self traverseResponderChainForKTStockDetailViewController:targetView];
}

- (id) traverseResponderChainForKTStockDetailViewController:(UIView *) targetView {
    id nextResponder = [targetView nextResponder];
    if ([nextResponder isKindOfClass:[KTStockDetailViewController class]]) {
        return nextResponder;
    } else if ([nextResponder isKindOfClass:[UIView class]]) {
        return [nextResponder traverseResponderChainForKTStockDetailViewController:targetView];
    } else {
        return nil;
    }
}
...
- (KTStockDetailViewController *) loadControllerAtIndex:(NSInteger) index andPlaceAtIndex:(NSInteger) destIndex {
    KTStockDetailViewController *viewController = dataSource(index);
    viewController.view.tag = 0;
  //  viewController.title(viewController.code);
